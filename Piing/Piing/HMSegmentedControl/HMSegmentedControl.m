...
- (void)commonInit {
    ...
    _verticalDividerColor = [UIColor grayColor];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    if (isAnimating)
    {
        [self.delegate didStartScroll:self Scroller:scrollView];
    }
}
...
- (CGSize)measureTitleAtIndex:(NSUInteger)index {
    ...
    if ([self.sectionTitles count] <= index)
    {
        return CGSizeZero;
    }
...
- (void)drawRect:(CGRect)rect {
    ...
    self.selectionIndicatorBoxLayer.cornerRadius = 5.0;
    ...
    if (self.type == HMSegmentedControlTypeText) {
        ...
        [self.sectionImages enumerateObjectsUsingBlock:^(id iconImage, NSUInteger idx, BOOL *stop) {
            ...
            float imgWidth = 20*MULTIPLYHEIGHT;
            float imgHeight = 20*MULTIPLYHEIGHT;
            
            CGFloat imageWidth = imgWidth;
            CGFloat imageHeight = imgHeight;
            ...
            float imgY = 15*MULTIPLYHEIGHT;
            CGRect imageRect = CGRectMake(imageXOffset, imgY, imageWidth, imageHeight);
            
            float textY = imgY+imageWidth+5*MULTIPLYHEIGHT;
            CGRect textRect = CGRectMake(textXOffset, textY, textWidth, stringHeight);
            ...
            float lblWidth = 18*MULTIPLYHEIGHT;
            CALayer *backgroundLayer = [CALayer layer];
            backgroundLayer.frame = CGRectMake(imageLayer.frame.size.width-8*MULTIPLYHEIGHT, -5*MULTIPLYHEIGHT, lblWidth, lblWidth);
            ...
            backgroundLayer.backgroundColor = [UIColor darkGrayColor].CGColor;
            //[imageLayer addSublayer:backgroundLayer];
            [imageLayer setValue:icon forKey:[NSString stringWithFormat:@"%d", 1]];
            [backgroundLayer setValue:icon forKey:[NSString stringWithFormat:@"%d", 2]];
            
            CATextLayer *lblBadge = [CATextLayer layer];
            [lblBadge setValue:icon forKey:[NSString stringWithFormat:@"%lu", (unsigned long)idx]];
            lblBadge.frame = CGRectMake(0, 2.5*MULTIPLYHEIGHT, lblWidth, lblWidth);
            [lblBadge setAlignmentMode:kCAAlignmentCenter];
            [lblBadge setTruncationMode:kCATruncationEnd];
            [lblBadge setForegroundColor:[UIColor whiteColor].CGColor];
            [lblBadge setFont:APPFONT_MEDIUM];
            [lblBadge setFontSize:appdel.FONT_SIZE_CUSTOM-1];
            [backgroundLayer addSublayer:lblBadge];
            
            if ([appdel.dictEachSegmentCount objectForKey:[NSString stringWithFormat:@"%lu", (unsigned long)idx]])
            {
                if ([[appdel.dictEachSegmentCount objectForKey:[NSString stringWithFormat:@"%lu", (unsigned long)idx]]isEqualToString:@"0"])
                {
                ...
                [lblBadge setString:[appdel.dictEachSegmentCount objectForKey:[NSString stringWithFormat:@"%lu", (unsigned long)idx]]];
            }
            else
            {
                backgroundLayer.hidden = YES;
            ...
            if (self.isVerticalDividerEnabled && idx > 0) {
                CALayer *verticalDividerLayer = [CALayer layer];
                verticalDividerLayer.frame = CGRectMake((self.segmentWidth * idx) - (self.verticalDividerWidth / 2), 20*MULTIPLYHEIGHT, self.verticalDividerWidth, self.frame.size.height-(15*MULTIPLYHEIGHT * 2));
                verticalDividerLayer.backgroundColor = self.verticalDividerColor.CGColor;
...
- (CGRect)frameForSelectionIndicator {
    ...
    if (self.selectionStyle == HMSegmentedControlSelectionStyleArrow) {
        ...
        CGRect rect = CGRectMake(x - (self.selectionIndicatorHeight / 2), indicatorYOffset, self.selectionIndicatorHeight * 2, self.selectionIndicatorHeight);
        ...
        if (self.selectionStyle == HMSegmentedControlSelectionStyleTextWidthStripe &&
            ...
            CGRect rect = CGRectMake(x + self.selectionIndicatorEdgeInsets.left, indicatorYOffset, sectionWidth - self.selectionIndicatorEdgeInsets.right, self.selectionIndicatorHeight);
            ...
            if (self.segmentWidthStyle == HMSegmentedControlSegmentWidthStyleDynamic) {
                ...
                CGRect rect = CGRectMake(selectedSegmentOffset + self.selectionIndicatorEdgeInsets.left, indicatorYOffset, [[self.segmentWidthsArray objectAtIndex:self.selectedSegmentIndex] floatValue] - self.selectionIndicatorEdgeInsets.right, self.selectionIndicatorHeight + self.selectionIndicatorEdgeInsets.bottom);
                
                return rect;
            }
            
            CGRect rect = CGRectMake((self.segmentWidth + self.selectionIndicatorEdgeInsets.left) * self.selectedSegmentIndex, indicatorYOffset, self.segmentWidth - self.selectionIndicatorEdgeInsets.right, self.selectionIndicatorHeight);;
...
- (CGRect)frameForFillerSelectionIndicator {
    if (self.segmentWidthStyle == HMSegmentedControlSegmentWidthStyleDynamic) {
        ...
        float backgroundY = 46*MULTIPLYHEIGHT;
        float backgroundHeight = 18*MULTIPLYHEIGHT;
        
        CGRect rect = CGRectMake(selectedSegmentOffset, backgroundY, [[self.segmentWidthsArray objectAtIndex:self.selectedSegmentIndex] floatValue], backgroundHeight);
    ...
    float backgroundY = 46*MULTIPLYHEIGHT;
    float backgroundHeight = 18*MULTIPLYHEIGHT;
    float minusWidth = 22*MULTIPLYHEIGHT;
    
    CGRect rect = CGRectMake((self.segmentWidth * self.selectedSegmentIndex)+(self.segmentWidth/2 - ([self measureTitleAtIndex:self.selectedSegmentIndex].width+minusWidth)/2), backgroundY, [self measureTitleAtIndex:self.selectedSegmentIndex].width+minusWidth, backgroundHeight);
...
- (void)updateSegmentsRects {
    ...
    float widthe = 10*MULTIPLYHEIGHT;
    
    self.scrollView.contentInset = UIEdgeInsetsMake(0, widthe, 0, widthe);
    ...
    if (self.type == HMSegmentedControlTypeText && self.segmentWidthStyle == HMSegmentedControlSegmentWidthStyleFixed) {
        ...
        [self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
            ...
            CGFloat simageW = 27*MULTIPLYHEIGHT;
            
            CGFloat imageWidth = simageW + self.segmentEdgeInset.left;
