...
@interface PriceListViewController_New () <CustomPopoverViewDelegate, HMSegmentedControlDelegate, UISearchBarDelegate>
{
    ...
    UIView *view_Bags, *view_Shoes, *view_Curtain;
    
    HMSegmentedControl *segmentCategory_Shoe, *segmentCategory_Bag, *segmentCategory_Curtain;
    
    UISegmentedControl *segmentControlShoe, *segmentControlBag, *segmentControlCurtain;
    ...
    UIView *view_Under_Shoes, *view_Under_Curtain;
    
    UIView *viewImagesShoe, *viewImageCurtain;
    ...
    UIButton *btnSwitch;
    UILabel *lblCurtainPrice;
}

@end


@implementation PriceListViewController_New
...
- (void)viewDidLoad {
    ...
    arrayTypes = [[NSArray alloc]initWithObjects:@"LOAD WASH", @"DRY CLEANING", @"GREEN DRY CLEANING", @"WASH & IRON", @"IRONING", @"LEATHER", @"CURTAIN CLEANING", @"CARPET CLEANING", @"BAGS", @"SHOES", nil];
    
    arrayServiceTypes = [[NSArray alloc]initWithObjects:SERVICETYPE_WF, SERVICETYPE_DC, SERVICETYPE_DCG, SERVICETYPE_WI, SERVICETYPE_IR, SERVICETYPE_LE, SERVICETYPE_CC_W_DC, SERVICETYPE_CA, SERVICETYPE_BAG, SERVICETYPE_SHOE_CLEAN, nil];
    ...
    if (!appDel.isPriceListFromTab && ![appDel.strServiceTypeFromHomeage length])
    {
        ...
        if (self.isFlipping)
        {
            [backBtn setImage:[UIImage imageNamed:@"cancel_grey"] forState:UIControlStateNormal];
        }
        else
        {
    ...
    btnPE.frame = CGRectMake(screen_width-btnWidth, yAxis, btnWidth, 40);
    [btnPE setImage:[UIImage imageNamed:@"price_estimator"] forState:UIControlStateNormal];
    [btnPE addTarget:self action:@selector(clickPriceEstimator) forControlEvents:UIControlEventTouchUpInside];
    ...
    view_Curtain = [[UIView alloc]initWithFrame:CGRectMake(0, yAxis, screen_width, screen_height-yAxis)];
    [self.view addSubview:view_Curtain];
    view_Curtain.hidden = YES;
...
//    [view_Express addSubview:mySwitch2];
    ...
    if (appDel.isPriceListFromTab)
    {
        view_Bg = [[UIView alloc]initWithFrame:CGRectMake(0, yAxis, screen_width, screen_height-yAxis-TAB_BAR_HEIGHT)];
    ...
    if ([self.selectedServiceTypeId length])
    {
        strJobType = SERVICETYPE_DCG;
        
        attr = [[NSMutableAttributedString alloc] initWithString:@"GREEN DRY CLEANING"];
    }
    else if ([appDel.strServiceTypeFromHomeage length])
    {
        if ([appDel.strServiceTypeFromHomeage isEqualToString:SERVICETYPE_SHOE_CLEAN])
        {
            ...
            attr = [[NSMutableAttributedString alloc] initWithString:@"SHOES"];
        }
        else
        {
            view_Bags.hidden = NO;
            ...
            strJobType = SERVICETYPE_BAG;
            
            attr = [[NSMutableAttributedString alloc] initWithString:@"BAGS"];
        }
        
        self.selectedServiceTypeId = strJobType;
        
        appDel.strServiceTypeFromHomeage = @"";
    }
    else
    {
        self.selectedServiceTypeId = SERVICETYPE_DC;
...
-(void) btnServiceTypeClicked:(UIButton *) sender
{
    ...
    else
    {
        ...
        customPopOverView = [[CustomPopoverView alloc]initWithArray:arrayTypes SelectedRow:[arrayServiceTypes indexOfObject:self.selectedServiceTypeId]];
...
-(void) didSelectFromList:(NSString *) string AtIndex:(NSInteger)row
{
    ...
    self.selectedServiceTypeId = [arrayServiceTypes objectAtIndex:row];
    ...
    if ([self.selectedServiceTypeId isEqualToString:SERVICETYPE_WF] || [self.selectedServiceTypeId isEqualToString:SERVICETYPE_CA])
    {
        view_SegmentBG.hidden = YES;
    ...
    else if ([self.selectedServiceTypeId isEqualToString:SERVICETYPE_SHOE_CLEAN] || [self.selectedServiceTypeId isEqualToString:SERVICETYPE_SHOE_POLISH])
    {
        strOrderType = ORDER_TYPE_REGULAR;
        //[mySwitch2 setOn:NO animated:YES];
        //segmentSwitch.selectedSegmentIndex = 0;
    ...
    else if ([self.selectedServiceTypeId isEqualToString:SERVICETYPE_BAG])
    {
        strOrderType = ORDER_TYPE_REGULAR;
        //[mySwitch2 setOn:NO animated:YES];
        //segmentSwitch.selectedSegmentIndex = 0;
    ...
    else if ([self.selectedServiceTypeId containsString:@"CC_"])
    {
        strOrderType = ORDER_TYPE_REGULAR;
        
        selectedCategoryIndex = 0;
        
        view_Curtain.hidden = NO;
        view_Shoes.hidden = YES;
        view_Bags.hidden = YES;
...
-(void) addBagData:(CGFloat) yAxis
{
    ...
    if ([appDel.strBagClean length])
    {
        strBagDesc = appDel.strBagClean;
    }
    else
    {
...
-(void) bagAddonService
{
    ...
    [WebserviceMethods sendRequestWithURLString:urlStr requestMethod:@"POST" withDetailsDictionary:registrationDetailsDic andResponseCallBack:^(NSURLResponse *response, NSError *error, id responseObj) {
        ...
        if ([responseObj objectForKey:@"s"] && [[responseObj objectForKey:@"s"] intValue] == 1)
        {
            ...
            {
                [arrayRow addObjectsFromArray:[[[responseObj objectForKey:@"em"] objectAtIndex:0] objectForKey:@"addOn"]];
            }
...
-(void) showCurtainCleaning
{
    for (__strong id view in view_Curtain.subviews)
    {
        [view removeFromSuperview];
        view = nil;
    }
    
    [self curtainCleaningData];
    
    CGFloat yAxis = 10*MULTIPLYHEIGHT;
    
//    NSArray *arraySer = @[@"WASH & IRON", @"DRY CLEANING"];
//    
...
//        [btnSer addTarget:self action:@selector(btnCurtainServiceTypeClicked:) forControlEvents:UIControlEventTouchUpInside];
//        [view_Curtain addSubview:btnSer];
//        
//        xAxis += size.width + 15*MULTIPLYHEIGHT + 30*MULTIPLYHEIGHT;
//        
//        if (([strJobType isEqualToString:SERVICETYPE_CC_W_WI] || [strJobType isEqualToString:SERVICETYPE_CC_WI])&& i == 0)
//        {
//            btnSer.selected = YES;
//            
//            btnSer.layer.borderColor = [UIColor grayColor].CGColor;
...
//        }
//        else if (([strJobType isEqualToString:SERVICETYPE_CC_W_DC] || [strJobType isEqualToString:SERVICETYPE_CC_DC]) && i == 1)
//        {
//            btnSer.selected = YES;
//            
//            btnSer.layer.borderColor = [UIColor grayColor].CGColor;
...
//    yAxis += btnH + 10*MULTIPLYHEIGHT;
    ...
    view_Under_Curtain = [[UIView alloc]initWithFrame:CGRectMake(0, yAxis, screen_width, view_Curtain.frame.size.height-yAxis)];
    view_Under_Curtain.backgroundColor = [[UIColor lightGrayColor] colorWithAlphaComponent:0.1];
    [view_Curtain addSubview:view_Under_Curtain];
    ...
    segmentCategory_Curtain = [[HMSegmentedControl alloc]initWithSectionTitles:arraykey];
    float left = 15*MULTIPLYHEIGHT;
    float right = 15*MULTIPLYHEIGHT;
    segmentCategory_Curtain.segmentEdgeInset = UIEdgeInsetsMake(0, left, 0, right);
    segmentCategory_Curtain.selectionIndicatorEdgeInsets = UIEdgeInsetsMake(0, 11*MULTIPLYHEIGHT, 0, 18*MULTIPLYHEIGHT);
    segmentCategory_Curtain.frame = CGRectMake(0, yAxis, screen_width, segmentCHeight);
    segmentCategory_Curtain.borderType = HMSegmentedControlBorderTypeNone;
    segmentCategory_Curtain.backgroundColor = [UIColor clearColor];
    segmentCategory_Curtain.segmentWidthStyle = HMSegmentedControlSegmentWidthStyleDynamic;
    segmentCategory_Curtain.selectionIndicatorLocation = HMSegmentedControlSelectionIndicatorLocationDown;
    segmentCategory_Curtain.selectionIndicatorColor = BLUE_COLOR;
    segmentCategory_Curtain.selectionIndicatorHeight = 2.0;
    segmentCategory_Curtain.titleTextAttributes = @{NSForegroundColorAttributeName : [UIColor lightGrayColor], NSFontAttributeName : [UIFont fontWithName:APPFONT_BOLD size:appDel.FONT_SIZE_CUSTOM-1]};
    segmentCategory_Curtain.selectedTitleTextAttributes = @{NSForegroundColorAttributeName : [UIColor darkGrayColor], NSFontAttributeName : [UIFont fontWithName:APPFONT_BOLD size:appDel.FONT_SIZE_CUSTOM-1]};
    [segmentCategory_Curtain addTarget:self action:@selector(segmentedControlCurtainChangedValue:) forControlEvents:UIControlEventValueChanged];
    [view_Under_Curtain addSubview:segmentCategory_Curtain];
    
    segmentCategory_Curtain.selectedSegmentIndex = selectedCategoryIndex;
    
    yAxis += segmentCHeight + 20*MULTIPLYHEIGHT;
    
    CGFloat lblIX = 20*MULTIPLYHEIGHT;
    CGFloat lblIH = 30*MULTIPLYHEIGHT;
    
    UILabel *lblInst = [[UILabel alloc]initWithFrame:CGRectMake(lblIX, yAxis, view_Under_Curtain.frame.size.width-lblIX*2, lblIH)];
    ...
    [view_Under_Curtain addSubview:lblInst];
    
    yAxis += lblIH+5*MULTIPLYHEIGHT;
    
    // Custom Switch
    
    CGFloat sgW = 120 * MULTIPLYHEIGHT;
    CGFloat sgH = 17 * MULTIPLYHEIGHT;
    
    segmentControlCurtain = [[UISegmentedControl alloc]initWithItems:@[@"YES", @"NO"]];
    segmentControlCurtain.frame = CGRectMake(screen_width/2-sgW/2, yAxis, sgW, sgH);
    [view_Under_Curtain addSubview:segmentControlCurtain];
    
    [[UISegmentedControl appearance] setTitleTextAttributes:@{NSForegroundColorAttributeName : [UIColor whiteColor], NSFontAttributeName : [UIFont fontWithName:APPFONT_BOLD size:appDel.FONT_SIZE_CUSTOM-3], NSKernAttributeName : @(1.0)} forState:UIControlStateSelected];
    [[UISegmentedControl appearance] setTitleTextAttributes:@{NSForegroundColorAttributeName : [UIColor blackColor], NSFontAttributeName : [UIFont fontWithName:APPFONT_BOLD size:appDel.FONT_SIZE_CUSTOM-3], NSKernAttributeName : @(1.0)} forState:UIControlStateNormal];
    
    [segmentControlCurtain addTarget:self action:@selector(segmentChangeForCurtain:) forControlEvents:UIControlEventValueChanged];
    segmentControlCurtain.tintColor = [UIColor colorWithRed:64.0/255.0 green:143.0/255.0 blue:210.0/255.0 alpha:1.0];
    segmentControlCurtain.selectedSegmentIndex = 0;
    
    if ([strJobType isEqualToString:SERVICETYPE_CC_WI] || [strJobType isEqualToString:SERVICETYPE_CC_DC])
    {
        segmentControlCurtain.selectedSegmentIndex = 1;
    }
    
//    segmentControlCurtain.layer.cornerRadius = 12.0;
//    segmentControlCurtain.layer.borderColor = BLUE_COLOR.CGColor;
//    segmentControlCurtain.layer.borderWidth = 0.6f;
//    segmentControlCurtain.layer.masksToBounds = YES;
    
    yAxis += sgH + 15*MULTIPLYHEIGHT;
    
    CGFloat imgSW = 60*MULTIPLYHEIGHT;
    CGFloat imgX = screen_width/2-imgSW/2;
    
    viewImageCurtain = [[UIView alloc]initWithFrame:CGRectMake(0, yAxis, screen_width, view_Under_Curtain.frame.size.height-yAxis)];
    [view_Under_Curtain addSubview:viewImageCurtain];
    
    CGFloat localYaxis = 10;
    
    UIImageView *imgShoe = [[UIImageView alloc]initWithFrame:CGRectMake(imgX, localYaxis, imgSW, imgSW)];
    imgShoe.image = [UIImage imageNamed:@"curtains"];
    imgShoe.contentMode = UIViewContentModeScaleAspectFit;
    [viewImageCurtain addSubview:imgShoe];
    
    
    localYaxis += imgSW+20*MULTIPLYHEIGHT;
    
    lblCurtainPrice = [[UILabel alloc]initWithFrame:CGRectMake(0, localYaxis, screen_width, 20)];
    lblCurtainPrice.numberOfLines = 0;
    lblCurtainPrice.textAlignment = NSTextAlignmentCenter;
    lblCurtainPrice.textColor = [UIColor grayColor];
    [viewImageCurtain addSubview:lblCurtainPrice];
    
    NSString *strPairPrice = [NSString stringWithFormat:@"$%.2f", [[[arrayRow objectAtIndex:0] objectForKey:@"ip"] floatValue]];
    
    NSString *strNext = @"";
    
    if ([[[arrayRow objectAtIndex:0] objectForKey:@"unit"] isEqualToString:@"kg"])
    {
        strNext = @"per kg";
    }
    else
    ...
    NSString *strFull = [NSString stringWithFormat:@"%@ %@", strPairPrice, strNext];
    
    NSMutableAttributedString *attrCurtainPrice = [[NSMutableAttributedString alloc]initWithString:strFull];
    
    [attrCurtainPrice addAttributes:@{NSForegroundColorAttributeName : [UIColor grayColor], NSFontAttributeName : [UIFont fontWithName:APPFONT_BOLD size:appDel.HEADER_LABEL_FONT_SIZE+2], NSKernAttributeName : @(1.0)} range:NSMakeRange(0, strPairPrice.length)];
    
    [attrCurtainPrice addAttributes:@{NSForegroundColorAttributeName : [UIColor grayColor], NSFontAttributeName : [UIFont fontWithName:APPFONT_REGULAR size:appDel.HEADER_LABEL_FONT_SIZE-3], NSKernAttributeName : @(0.6)} range:NSMakeRange(strPairPrice.length+1, strNext.length)];
    
    lblCurtainPrice.attributedText = attrCurtainPrice;
    
    CGSize size = [AppDelegate getAttributedTextHeightForText:attrCurtainPrice WithWidth:lblCurtainPrice.frame.size.width];
    
    CGRect rect = lblCurtainPrice.frame;
    rect.size.height = size.height;
    lblCurtainPrice.frame = rect;
    
    localYaxis += lblCurtainPrice.frame.size.height;
    
    lblDays = [[UILabel alloc]initWithFrame:CGRectMake(0, localYaxis, screen_width, 20*MULTIPLYHEIGHT)];
    lblDays.textAlignment = NSTextAlignmentCenter;
    [viewImageCurtain addSubview:lblDays];
    ...
    localYaxis += lblDays.frame.size.height+10*MULTIPLYHEIGHT;
    ...
    lblDesc.textAlignment = NSTextAlignmentCenter;
    lblDesc.textColor = [UIColor grayColor];
    [viewImageCurtain addSubview:lblDesc];
    ...
    size = [AppDelegate getAttributedTextHeightForText:lblAttr WithWidth:lblDesc.frame.size.width];
...
-(void) curtainCleaningData
{
    [arraykey removeAllObjects];
    
    for (int i=0; i<[arrayCategory count]; i++)
    {
        NSDictionary *dict = [arrayCategory objectAtIndex:i];
        
        [arraykey addObject:[dict objectForKey:@"n"]];
    }
    ...
    if ([arrayCategory count])
    {
        ...
        [arrayRow addObject:dictRow];
    }
}

-(void) btnCurtainServiceTypeClicked:(UIButton *) sender
{
    UIButton *btn1 = [view_Curtain viewWithTag:1];
    UIButton *btn2 = [view_Curtain viewWithTag:2];
    ...
    if (btn1 == sender)
    {
        self.selectedServiceTypeId = SERVICETYPE_CC_W_WI;
        
        strJobType = SERVICETYPE_CC_W_WI;
    ...
    {
        self.selectedServiceTypeId = SERVICETYPE_CC_W_DC;
        
        strJobType = SERVICETYPE_CC_W_DC;
...
-(void) segmentChangeForCurtain:(UISegmentedControl *) seg
{
    ...
    {
        self.selectedServiceTypeId = SERVICETYPE_CC_DC;
        
        strJobType = SERVICETYPE_CC_DC;
    }
    
    [self getCategories];
}

-(void) segmentedControlCurtainChangedValue:(HMSegmentedControl *) segment
{
...
//    }
    ...
    if ([strJobType isEqualToString:SERVICETYPE_SHOE_CLEAN])
    {
        strShoeDesc = appDel.strShoeClean;
    }
    else
    {
        strShoeDesc = appDel.strShoePolish;
    }
...
-(void) btnShoeServiceTypeClicked:(UIButton *) sender
{
    ...
    if (btn1 == sender)
    {
        self.selectedServiceTypeId = SERVICETYPE_SHOE_CLEAN;
    ...
    {
        self.selectedServiceTypeId = SERVICETYPE_SHOE_POLISH;
...
-(void) clickPriceEstimator
{
    [FIRAnalytics logEventWithName:@"pricing__screen_estimator_button" parameters:nil];
    
    PriceEstimatorViewController_New *pe = [[PriceEstimatorViewController_New alloc]init];
    pe.hasBackButton = YES;
...
-(void) viewWillAppear:(BOOL)animated
{
    ...
    [FIRAnalytics logEventWithName:@"pricing_screen_button" parameters:nil];
...
//    if (![[NSUserDefaults standardUserDefaults] objectForKey:@"Price_Jobtypes"])
//    {
//        [[NSUserDefaults standardUserDefaults] setObject:@"YES" forKey:@"Price_Jobtypes"];
//        [[NSUserDefaults standardUserDefaults] synchronize];
//        
//        [self performSelector:@selector(closeCustomPopover) withObject:nil afterDelay:2.0f];
//    }
}
...
- (void)backToPreviousScreen
{
    ...
    else if (self.isFromWelcome)
    {
        [self dismissViewControllerAnimated:YES completion:^{
            
            [self.delegate didDismissPricingView];
