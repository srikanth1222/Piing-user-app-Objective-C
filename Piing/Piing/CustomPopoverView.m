...
-(id) initWithArray:(NSArray *)arrayData IsAddressType:(BOOL) isAddressType
{
    if (self == [super init])
    {
        ...
        if (isAddressType)
        {
            [self.arrayList addObject:@"+ ADD ADDRESS"];
        }
...
-(id) initWithArray:(NSArray *)arrayData IsPaymentType:(BOOL) isPaymentType
{
    if (self == [super init])
    {
        ...
        if (isPaymentType)
        {
            [self.arrayList addObject:@"+ ADD CARD"];
        }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    UIImageView *imgCard = (UIImageView *) [cell.contentView viewWithTag:11];
    ...
    {
        if ([self.arrayList count]-1 == indexPath.row)
        {
    ...
    else if (self.isPaymentSelected)
    {
        ...
        else
        {
            ...
            [imgCard sd_setImageWithURL:[NSURL URLWithString:[[self.arrayList objectAtIndex:indexPath.row] objectForKey:@"cardTypeImage"]]
    ...
    if([self.delegate respondsToSelector:@selector(didSelectFromList:AtIndex:)])
    {
        ...
        if (self.isAddressSelected)
        {
            if ([self.arrayList count]-1 == indexPath.row)
            {
                ...
                else if (self.isFromTag == 2)
                {
                    superViewController = (UIViewController *) [self.superview.superview nextResponder];
                }
                else if (self.isFromTag == 3)
                {
                    superViewController = (UIViewController *) [self.superview.superview.superview nextResponder];
                }
                
                addAddVC.delegate = superViewController;
                [superViewController presentViewController:addAddVC animated:YES completion:nil];
        ...
        else if (self.isPaymentSelected)
        {
            if ([self.arrayList count]-1 == indexPath.row)
            {
                ...
                CardIOPaymentViewController *scanViewController = [[CardIOPaymentViewController alloc] initWithPaymentDelegate:superViewController];
                
                [superViewController presentViewController:scanViewController animated:YES completion:nil];
