    ...
    if ([self.strPrefs length])
    {
        ...
        for (NSString *str in arr)
        {
            ...
            if ([dict1 objectForKey:@"name"])
            {
                ...
                {
                    [dictMain setObject:@"" forKey:[dict1 objectForKey:@"name"]];
                }
            }
        }
    }
    
    NSArray *arrayKeys = @[@"Shirts", @"TrousersHanged", @"TrousersCrease", @"Starch", @"Stain", @"Note"];
    NSArray *arrayValues = @[@"Hanger", @"Standard Hanger", @"With Crease", @"No Starch", @"No", @""];
    
    for (int i = 0; i < [arrayKeys count]; i++)
    {
        if (![dictMain objectForKey:[arrayKeys objectAtIndex:i]])
        {
            [dictMain setObject:[arrayValues objectAtIndex:i] forKey:[arrayKeys objectAtIndex:i]];
        }
    }
...
//    [dictMain setObject:@"" forKey:@"Note"];
        ...
        if (i == 0 || i == 1 || i == 3)
        {
            ...
            for (int j=0; j<2; j++)
            {
                ...
                if (i == 0 && j == 0)
                {
                    ...
                    NSString *strClipHanger_Extra = [NSString stringWithFormat:@"Additional $%.2f cents per fold", appDel.fold_ExtraCharge];
                ...
                else if (i == 1 && j == 0)
                {
                    ...
                    NSString *strClipHanger_Extra = [NSString stringWithFormat:@"Additional $%.2f cents per trouser", appDel.clipHanger_ExtraCharge];
    ...
    btnDone.titleLabel.font = [UIFont fontWithName:APPFONT_BOLD size:appDel.FONT_SIZE_CUSTOM-1];
...
//    btnDone.titleEdgeInsets = titleInsets;
    ...
    btnDone.frame = CGRectMake(btnDX, screen_height-50*MULTIPLYHEIGHT, screen_width-btnDX*2, btnDH);
    
    if (self.isFromOrderDetails)
    {
        //[self checkAllowUpdateFields];
    }
    else if (self.isFromUpdateOrder)
    {
//        if ([self.dictAllowFields count] && [[self.dictAllowFields objectForKey:@"prf"] caseInsensitiveCompare:@"n"] == NSOrderedSame)
//        {
...
//    }
    ...
    if (self.notEditable)
    {
        [appDel showAlertWithMessage:@"Preferences not editable" andTitle:@"" andBtnTitle:@"OK"];
    ...
    if (self.isFromMoreScreen)
    {
        ...
        NSString *urlStr = [NSString stringWithFormat:@"%@user/preferences/save", BASE_URL];
        ...
        [WebserviceMethods sendRequestWithURLString:urlStr requestMethod:@"POST" withDetailsDictionary:dic andResponseCallBack:^(NSURLResponse *response, NSError *error, id responseObj) {
            ...
            if([[responseObj objectForKey:@"s"] intValue] == 1)
            {
                ...
                UIAlertController * alert = [UIAlertController
                                             ...
                                             message:[responseObj objectForKey:@"msg"]
